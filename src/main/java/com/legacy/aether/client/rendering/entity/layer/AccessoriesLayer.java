...
public class AccessoriesLayer implements LayerRenderer<AbstractClientPlayer>
{
	...
	@Override
	public void render(AbstractClientPlayer player, float limbSwing, float prevLimbSwing, float partialTicks, float rotation, float interpolateRotation, float prevRotationPitch, float scale)
	{
		...
		if (accessories.stacks.get(0).getItem() instanceof ItemAccessory)
		{
			...
			Minecraft.getInstance().getTextureManager().bindTexture(pendant.getAccessoryTexture());
		...
		if (accessories.stacks.get(1).getItem() instanceof ItemAccessory && accessories.stacks.get(1).getItem() != ItemsAether.invisibility_cape)
		{
	        ...
	        if (player.hasPlayerInfo() && !player.isInvisible())
	        {
				...
				{
					Minecraft.getInstance().getTextureManager().bindTexture(Aether.locate("textures/armor/accessory_swuff.png"));
				}
				else
				{
					Minecraft.getInstance().getTextureManager().bindTexture(cape.getAccessoryTexture());
				}
		...
		if (accessories.stacks.get(6).getItem() instanceof ItemAccessory)
		{
			...
			Minecraft.getInstance().getTextureManager().bindTexture(gloves.getAccessoryTexture());
		...
		if (accessories.stacks.get(2).getItem() instanceof ItemAccessory)
		{
			...
			Minecraft.getInstance().getTextureManager().bindTexture(shield.getAccessoryTexture());
		...
		{
			Minecraft.getInstance().getTextureManager().bindTexture(TEXTURE_VALKYRIE);
