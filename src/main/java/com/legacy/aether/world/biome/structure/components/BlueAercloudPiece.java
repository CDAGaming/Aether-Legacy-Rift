...
public class BlueAercloudPiece extends StructurePiece
{
    ...
    public BlueAercloudPiece()
    {

    }

	public BlueAercloudPiece(SharedSeedRandom random, int chunkX, int chunkY, int chunkZ)
	{
                ...
                for (int y1 = y; y1 < y + this.randY + 2; ++y1)
                {
                    for (int z1 = z; z1 < z + this.randZ + 3; ++z1)
                    {
                        if (this.getBlockStateFromPos(worldIn, x1, y1, z1, boundingBoxIn).isAir())
                        {
                        	...
                        	{
                        		this.setBlockState(worldIn, BlocksAether.blue_aercloud.getDefaultState(), x1, y1, z1, boundingBoxIn);
                        	}
                        }
                    }
                }
